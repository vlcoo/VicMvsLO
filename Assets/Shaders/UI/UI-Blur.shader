            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                for (int x = lower; x <= upper; x++)
                {
                    for (int y = lower; y <= upper; y++)
                    {
                        ...
                        half2 pos = half2((half) x / _Iterations * _Distance, (half) y / _Iterations * _Distance);
                        color += tex2D(_CameraOpaqueTexture, (IN.screenPosition.xy / IN.screenPosition.w) + pos);
                    }
                }
