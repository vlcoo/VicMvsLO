    ...
    public class Room : RoomInfo
    {
        ...
        public new bool IsVisible
        {
            ...
            set
            {
                if (value != isVisible)
                    if (!IsOffline)
                        LoadBalancingClient.OpSetPropertiesOfRoom(
                            new Hashtable { { GamePropertyKey.IsVisible, value } });
        ...
        /// <summary>
        ///     List of users who are expected to join this room. In matchmaking, Photon blocks a slot for each of these UserIDs
        ///     out of the MaxPlayers.
        /// </summary>
        /// <remarks>
        ///     The corresponding feature in Photon is called "Slot Reservation" and can be found in the doc pages.
        ///     Define expected players in the methods: <see cref="LoadBalancingClient.OpCreateRoom" />,
        ///     <see cref="LoadBalancingClient.OpJoinRoom" /> and <see cref="LoadBalancingClient.OpJoinRandomRoom" />.
        /// </remarks>
        public string[] ExpectedUsers => expectedUsers;

        /// <summary>
        ///     Player Time To Live. How long any player can be inactive (due to disconnect or leave) before the user gets
        ///     removed from the playerlist (freeing a slot).
        /// </summary>
        public int PlayerTtl
        {
            ...
            set
            {
                if (value != playerTtl)
                    if (!IsOffline)
                        LoadBalancingClient.OpSetPropertyOfRoom(GamePropertyKey.PlayerTtl,
        ...
        /// <summary>
        ///     Room Time To Live. How long a room stays available (and in server-memory), after the last player becomes
        ...
        public int EmptyRoomTtl
        {
            ...
            set
            {
                if (value != emptyRoomTtl)
                    if (!IsOffline)
                        LoadBalancingClient.OpSetPropertyOfRoom(GamePropertyKey.EmptyRoomTtl,
                            value); // TODO: implement Offline Mode
        ...
        ///     This list is defined when creating the room and can't be changed afterwards. Compare:
        ///     LoadBalancingClient.OpCreateRoom()
        /// </summary>
        /// <remarks>
        ///     You could name properties that are not set from the beginning. Those will be synced with the lobby when added
        ///     later on.
        /// </remarks>
        public string[] PropertiesListedInLobby
        {
            get => propertiesListedInLobby;

            private set => propertiesListedInLobby = value;
        }
        ...
        ///     Within PUN, auto cleanup of events means that cached RPCs and instantiated networked objects are deleted from the
        ///     room.
        /// </remarks>
        ...
        /// <summary>
        ///     Extends SuppressRoomEvents: Define if Join and Leave events but also the actors' list and their respective
        ///     properties should not be sent to clients.
        /// </summary>
        public bool SuppressPlayerInfo { get; private set; }

        /// <summary>
        ///     Define if UserIds of the players are broadcast in the room. Useful for FindFriends and reserving slots for
        ///     expected users.
        /// </summary>
        ...
        /// <summary>
        ///     Creates a Room (representation) with given name and properties and the "listing options" as provided by
        ///     parameters.
        /// </summary>
        ...
        {
            BroadcastPropertiesChangeToAll = (roomFlags & (int)RoomOptionBit.BroadcastPropsChangeToAll) != 0;
            SuppressRoomEvents = (roomFlags & (int)RoomOptionBit.SuppressRoomEvents) != 0;
            SuppressPlayerInfo = (roomFlags & (int)RoomOptionBit.SuppressPlayerInfo) != 0;
            PublishUserId = (roomFlags & (int)RoomOptionBit.PublishUserId) != 0;
            DeleteNullProperties = (roomFlags & (int)RoomOptionBit.DeleteNullProps) != 0;
#if SERVERSDK
            this.CheckUserOnJoin = (roomFlags & (int)RoomOptionBit.CheckUserOnJoin) != 0;
#endif
            autoCleanUp = (roomFlags & (int)RoomOptionBit.DeleteCacheOnLeave) != 0;
        }

        protected internal override void InternalCacheProperties(Hashtable propertiesToCache)
        {
            var oldMasterId = masterClientId;
            ...
            if (oldMasterId != 0 && masterClientId != oldMasterId)
        ...
        /// </param>
        /// <param name="webFlags">
        ///     Defines if this SetCustomProperties-operation gets forwarded to your WebHooks. Client must be in
        ///     room.
        /// </param>
        /// <returns>
        ...
        {
            if (propertiesToSet == null || propertiesToSet.Count == 0) return false;
            var customProps = propertiesToSet.StripToStringKeys();

            if (IsOffline)
            {
                if (customProps.Count == 0) return false;
            ...
            else
            {
                ...
                return LoadBalancingClient.OpSetPropertiesOfRoom(customProps, expectedProperties, webFlags);
            }
        ...
        public bool SetPropertiesListedInLobby(string[] lobbyProps)
        {
            ...
            return LoadBalancingClient.OpSetPropertiesOfRoom(customProps);
        }
        ...
        public bool SetMasterClient(Player masterClientPlayer)
        {
            ...
            var prevProps = new Hashtable { { GamePropertyKey.MasterClientId, MasterClientId } };
            return LoadBalancingClient.OpSetPropertiesOfRoom(newProps, prevProps);
        }
        ...
        {
            var idToFind = findMaster && id == 0 ? MasterClientId : id;

            Player result = null;
            Players.TryGetValue(idToFind, out result);
        ...
        {
            if (ExpectedUsers == null || ExpectedUsers.Length == 0) return false;
        ...
        {
            return string.Format("Room: '{0}' {1},{2} {4}/{3} players.", name, isVisible ? "visible" : "hidden",
                isOpen ? "open" : "closed", maxPlayers, PlayerCount);
        }
        ...
        public new string ToStringFull()
        {
            return string.Format("Room: '{0}' {1},{2} {4}/{3} players.\ncustomProps: {5}", name,
                isVisible ? "visible" : "hidden", isOpen ? "open" : "closed", maxPlayers, PlayerCount,
                CustomProperties.ToStringFull());
        }
    }
}