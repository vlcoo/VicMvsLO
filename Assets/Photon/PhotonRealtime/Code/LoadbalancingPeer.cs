...
namespace Photon.Realtime
{
    ...
    /// <summary>
    ///     A LoadBalancingPeer provides the operations and enum definitions needed to use the LoadBalancing server application
    ///     which is also used in Photon Cloud.
    /// </summary>
    /// <remarks>
    ...
    public class LoadBalancingPeer : PhotonPeer
    {
        ...
        {
            get => RegionHandler.PingImplementation;
        ...
        public virtual bool OpGetRegions(string appId)
        {
            ...
            parameters[ParameterCode.ApplicationId] = appId;

            return SendOperation(OperationCode.GetRegions, parameters,
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpJoinLobby()");
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpLeaveLobby()");

            return SendOperation(OperationCode.LeaveLobby, (Dictionary<byte, object>)null, SendOptions.SendReliable);
        }
        ...
        {
            if (roomOptions == null) roomOptions = new RoomOptions();
            ...
            if (roomOptions.MaxPlayers > 0) gameProperties[GamePropertyKey.MaxPlayers] = roomOptions.MaxPlayers;
            ...
            if (roomOptions.EmptyRoomTtl > 0) op[ParameterCode.EmptyRoomTTL] = roomOptions.EmptyRoomTtl; //TURNBASED
            ...
            if (roomOptions.SuppressPlayerInfo) flags = flags | (int)RoomOptionBit.SuppressPlayerInfo;

            if (roomOptions.Plugins != null) op[ParameterCode.Plugins] = roomOptions.Plugins;
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpCreateRoom()");

            var op = new Dictionary<byte, object>();

            if (!string.IsNullOrEmpty(opParams.RoomName)) op[ParameterCode.RoomName] = opParams.RoomName;
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpJoinRoom()");
            var op = new Dictionary<byte, object>();
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpJoinRandomRoom()");
            ...
            if (expectedRoomProperties.Count > 0) opParameters[ParameterCode.GameProperties] = expectedRoomProperties;
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpJoinRandomOrCreateRoom()");
            ...
            if (createRoomParams != null)
            {
                ...
                RoomOptionsToOpParameters(opParameters, createRoomParams.RoomOptions, true);
            }
        ...
        ///     Async games can be re-joined (loaded) later on. Set to false, if you want to abandon a
        ///     game entirely.
        /// </param>
        ...
        {
            var opParameters = new Dictionary<byte, object>();
            if (becomeInactive) opParameters[ParameterCode.IsInactive] = true;
            if (sendAuthCookie) opParameters[ParameterCode.EventForward] = WebFlags.SendAuthCookieConst;
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpGetGameList()");
            ...
            {
                if (DebugOut >= DebugLevel.INFO)
                    Listener.DebugReturn(DebugLevel.INFO,
            ...
            opParameters[ParameterCode.Data] = queryData;

            return SendOperation(OperationCode.GetGameList, opParameters, SendOptions.SendReliable);
        }

        /// <summary>
        ///     Request the rooms and online status for a list of friends (each client must set a unique username via
        ///     OpAuthenticate).
        /// </summary>
        /// <remarks>
        ...
        public virtual bool OpFindFriends(string[] friendsToFind, FindFriendsOptions options = null)
        {
            ...
            if (options != null) opParameters[ParameterCode.FindFriendsOptions] = options.ToIntFlags();
        ...
        ///     If set, these must be in the current properties-set (on the server) to set
        ///     actorProperties: CAS.
        /// </param>
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpSetPropertiesOfActor()");
        ...
        {
            if (DebugOut >= DebugLevel.INFO) Listener.DebugReturn(DebugLevel.INFO, "OpSetPropertiesOfRoom()");
        ...
        ///     The client's version (clients with differing client appVersions are separated and players
        ///     don't meet).
        /// </param>
        /// <param name="authValues">
        ///     Contains all values relevant for authentication. Even without account system (external Custom
        ///     Auth), the clients are allowed to identify themselves.
        /// </param>
            ...
            string regionCode, bool getLobbyStatistics)
        {
            ...
            if (authValues != null && authValues.Token != null)
            {
                ...
                return SendOperation(OperationCode.Authenticate, opParameters,
            ...
            if (!string.IsNullOrEmpty(regionCode)) opParameters[ParameterCode.Region] = regionCode;
        ...
        /// </param>
        /// <param name="authValues">
        ///     Optional authentication values. The client can set no values or a UserId or some parameters
        ///     for Custom Authentication by a server.
        /// </param>
        /// <param name="regionCode">Optional region code, if the client should connect to a specific Photon Cloud Region.</param>
            ...
            string regionCode, EncryptionMode encryptionMode, ConnectionProtocol expectedProtocol)
        {
            ...
            {
                opParameters[ParameterCode.Token] = authValues.Token;
                return SendOperation(OperationCode.AuthenticateOnce, opParameters,
                    SendOptions.SendReliable); // we don't have to encrypt, when we have a token (which is encrypted)
            }
        ...
        {
            if (DebugOut >= DebugLevel.ALL) Listener.DebugReturn(DebugLevel.ALL, "OpChangeGroups()");

            var opParameters = new Dictionary<byte, object>();
            if (groupsToRemove != null) opParameters[ParameterCode.Remove] = groupsToRemove;
            if (groupsToAdd != null) opParameters[ParameterCode.Add] = groupsToAdd;
        ...
        /// </summary>
        /// <remarks>
        ///     This override explicitly uses another parameter order to not mix it up with the implementation for Hashtable
        ///     only.
        /// </remarks>
        ...
        {
            var paramDict = paramDictionaryPool.Acquire();
            try
            {
                if (raiseEventOptions != null)
                {
                    ...
                    switch (raiseEventOptions.CachingOption)
                    {
                        ...
                        case EventCaching.SlicePurgeUpToIndex:
                            ...
                            return SendOperation(OperationCode.RaiseEvent, paramDict, sendOptions);
                        ...
                        default:
                            if (raiseEventOptions.TargetActors != null)
                                ...
                                paramDict.Add(ParameterCode.Group, raiseEventOptions.InterestGroup);
                            ...
                            if (raiseEventOptions.Flags.HttpForward)
                                paramDict.Add(ParameterCode.EventForward, raiseEventOptions.Flags.WebhookFlags);
                            break;
                    }
                }

                paramDict.Add(ParameterCode.Code, eventCode);
                if (customEventContent != null) paramDict.Add(ParameterCode.Data, customEventContent);
                return SendOperation(OperationCode.RaiseEvent, paramDict, sendOptions);
            }
            finally
            {
                paramDictionaryPool.Release(paramDict);
            }
        }
        ...
        {
            if (DebugOut >= DebugLevel.ALL) Listener.DebugReturn(DebugLevel.ALL, "OpSettings()");

            var opParameters = new Dictionary<byte, object>();

            // implementation for Master Server:
            if (receiveLobbyStats) opParameters[0] = receiveLobbyStats;
    ...
    public class FindFriendsOptions
    {
        ...
        {
            var optionFlags = 0;
            if (CreatedOnGs) optionFlags = optionFlags | 0x1;
            if (Visible) optionFlags = optionFlags | 0x2;
            if (Open) optionFlags = optionFlags | 0x4;
            return optionFlags;
        }
    }
    ...
    ///     <see cref="https://doc.photonengine.com/en-us/pun/current/manuals-and-demos/matchmaking-and-lobby" />.
    /// </remarks>
    public class OpJoinRandomRoomParams
    {
        /// <summary>
        ///     The custom room properties a room must have to fit. All key-values must be present to match. In SQL Lobby, use
        ///     SqlLobbyFilter instead.
        /// </summary>
        ...
        /// <remarks>
        ///     See:
        ///     https://doc.photonengine.com/en-us/pun/v2/lobby-and-matchmaking/matchmaking-and-lobby#matchmaking_slot_reservation
        /// </remarks>
    ...
    {
        /// <summary>
        ///     A list of users who are expected to join the room along with this client. Reserves slots for rooms with
        ///     MaxPlayers value.
        /// </summary>
        public string[] ExpectedUsers;
        ...
        /// <summary>
        ///     The name of the room to create. If null, the server generates a unique name. If not null, it must be unique
        ///     and new or will cause an error.
        /// </summary>
    ...
    public class ErrorCode
    {
        ...
        ///     The operation you called is not implemented on the server (application). Server-side plugins affect the available
        ///     operations.
        /// </remarks>
        ...
        /// <summary>
        ///     (32762) All servers are busy. This is a temporary issue and the game logic should try again after a brief wait
        ///     time.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        ///     (32753) The Authentication ticket expired. Usually, this is refreshed behind the scenes. Connect (and
        ///     authorize) again.
        /// </summary>
        public const int AuthenticationTicketExpired = 0x7FF1;

        /// <summary>
        ///     (32752) A server-side plugin (or webhook) failed to execute and reported an error. Check the
        ...
        /// <summary>
        ///     (32749)  for join requests. Indicates the list of InactiveActors already contains an actor with the requested
        ///     ActorNr or UserId.
        /// </summary>
        public const int JoinFailedFoundInactiveJoiner = 32749; // 0x7FFF - 18,

        /// <summary>
        ///     (32748) for join requests. Indicates the list of Actors (active and inactive) did not contain an actor with the
        ///     requested ActorNr or UserId.
        /// </summary>
        ...
        /// <summary>
        ///     (32746) for join requests. Indicates the list of ActiveActors already contains an actor with the requested ActorNr
        ///     or UserId.
        /// </summary>
        public const int JoinFailedFoundActiveJoiner = 32746; // 0x7FFF - 21,

        /// <summary>
        ///     (32745)  for SetProerties and Raisevent (if flag HttpForward is true) requests. Indicates the maximum allowd http
        ///     requests per minute was reached.
        /// </summary>
    ...
    public class GamePropertyKey
    {
        ...
        /// <summary>
        ///     (250) A list of the room properties to pass to the RoomInfo list in a lobby. This is used in CreateRoom, which
        ///     defines this list once per room.
        /// </summary>
        ...
        /// <summary>
        ///     (248) Code for MasterClientId, which is synced by server. When sent as op-parameter this is (byte)203. As room
        ///     property this is (byte)248.
        /// </summary>
        ...
        /// <summary>
        ///     (246) Player Time To Live. How long any player can be inactive (due to disconnect or leave) before the user
        ///     gets removed from the playerlist (freeing a slot).
        /// </summary>
        public const byte PlayerTtl = 246;

        /// <summary>
        ///     (245) Room Time To Live. How long a room stays available (and in server-memory), after the last player becomes
        ///     inactive. After this time, the room gets persisted or destroyed.
        /// </summary>
    ...
    public class EventCode
    {
        ...
        /// <summary>
        ///     (253) When you call OpSetProperties with the broadcast option "on", this event is fired. It contains the
        ...
        /// <summary>
        ///     (251) Sent by Photon Cloud when a plugin-call or webhook-call failed or events cache limit exceeded. Usually,
    ...
    {
        /// <summary>
        ///     (237) A bool parameter for creating games. If set to true, no room events are sent to the clients on join and
        ///     leave. Default: false (and not sent).
        /// </summary>
        public const byte SuppressRoomEvents = 237;

        /// <summary>
        ///     (236) Time To Live (TTL) for a room when the last player leaves. Keeps room in memory for case a player
        ///     re-joins soon. In milliseconds.
        /// </summary>
        public const byte EmptyRoomTTL = 236;

        /// <summary>
        ///     (235) Time To Live (TTL) for an 'actor' in a room. If a client disconnects, this actor is inactive first and
        ///     removed after this timeout. In milliseconds.
        /// </summary>
        public const byte PlayerTTL = 235;

        /// <summary>
        ///     (234) Optional parameter of OpRaiseEvent and OpSetCustomProperties to forward the event/operation to a
        ///     web-service.
        /// </summary>
        public const byte EventForward = 234;

        /// <summary>
        ///     (233) Optional parameter of OpLeave in async games. If false, the player does abandons the game (forever). By
        ///     default players become inactive and can re-join.
        /// </summary>
        [Obsolete("Use: IsInactive")] public const byte IsComingBack = 233;

        /// <summary>
        ///     (233) Used in EvLeave to describe if a user is inactive (and might come back) or not. In rooms with PlayerTTL,
        ///     becoming inactive is the default case.
        /// </summary>
        ...
        /// <remarks>
        ///     This is not the same as the Operation's code, which is no longer sent as part of the parameter Dictionary in
        ///     Photon 3.
        /// </remarks>
        ...
        /// <summary>
        ///     (239) Used in Op Join to define if UserIds of the players are broadcast in the room. Useful for FindFriends
        ///     and reserving slots for expected users.
        /// </summary>
        public const byte PublishUserId = 239;

        /// <summary>
        ///     (238) The "Add" operation-parameter can be used to add something to some list or set. E.g. add groups to
        ///     player's interest groups.
        /// </summary>
        ...
        /// <summary>
        ///     (215) The JoinMode enum defines which variant of joining a room will be executed: Join only if available,
        ///     create if not exists or re-join.
        /// </summary>
        ...
        /// <summary>
        ///     (213) Used in matchmaking-related methods and when creating a room to name a lobby (to join or to attach a
        ///     room to).
        /// </summary>
        public const byte LobbyName = 213;

        /// <summary>
        ///     (212) Used in matchmaking-related methods and when creating a room to define the type of a lobby. Combined
        ///     with the lobby name this identifies the lobby.
        /// </summary>
        public const byte LobbyType = 212;

        /// <summary>
        ///     (211) This (optional) parameter can be sent in Op Authenticate to turn on Lobby Stats (info about lobby names
        ///     and their user- and game-counts).
        /// </summary>
        ...
        /// <summary>
        ///     (202) Used by the server in Operation Responses, when it sends the nickname of the client (the user's
        ///     nickname).
        /// </summary>
    ...
    /// <summary>
    ///     Defines possible values for OpJoinRoom and OpJoinOrCreate. It tells the server if the room can be only be
    ///     joined normally, created implicitly or found on a web-service for Turnbased games.
    /// </summary>
    ...
    public enum MatchmakingMode : byte
    {
        ...
        /// <summary>
        ///     Distributes players across available rooms sequentially but takes filter into account. Without filter, rooms
        ///     get players evenly distributed.
        /// </summary>
        SerialMatching = 1,

        /// <summary>
        ///     Joins a (fully) random room. Expected properties must match but aside from this, any available room might be
        ///     selected.
        /// </summary>
        RandomMatching = 2
    }
    ...
    public enum EventCaching : byte
    {
        ...
        /// <summary>
        ///     Purge cache slice with index. Exactly one slice is removed from cache. You must set
        ///     RaiseEventOptions.CacheSliceIndex for this.
        /// </summary>
        SlicePurgeIndex = 12,

        /// <summary>
        ///     Purge cache slices with specified index and anything lower than that. You must set
        ///     RaiseEventOptions.CacheSliceIndex for this.
        /// </summary>
    ...
    public class RoomOptions
    {
        ...
        /// <summary>
        ///     Time To Live (TTL) for an 'actor' in a room. If a client disconnects, this actor is inactive first and removed
        ///     after this timeout. In milliseconds.
        /// </summary>
        public int PlayerTtl;

        /// <summary>
        ///     Time To Live (TTL) for a room when the last player leaves. Keeps room in memory for case a player re-joins
        ///     soon. In milliseconds.
        /// </summary>
        ...
        /// <summary>
        ///     Defines if the UserIds of players get "published" in the room. Useful for FindFriends, if players want to play
        ...
        /// <remarks>
        ///     When Op SetProperties is setting a key's value to null, the server and clients should remove the key/value from the
        ///     Custom Properties.
        ///     By default, the server keeps the keys (and null values) and sends them to joining players.
        ///     Important: Only when SetProperties does a "broadcast", the change (key, value = null) is sent to clients to update
        ///     accordingly.
        ...
        /// <summary>
        ///     By default, property changes are sent back to the client that's setting them to avoid de-sync when properties
        ///     are set concurrently.
        /// </summary>
        /// <remarks>
        ///     This option is enables by default to fix this scenario:
        ///     1) On server, room property ABC is set to value FOO, which triggers notifications to all the clients telling them
        ...
        ///     4) Server receives the set operation and changes the official value of ABC to BAR, but never notifies the client
        ///     that sent the set operation that the value is now BAR.
        ///     Without this option, the client that set the value to BAR never hears from the server that the official copy has
        ///     been updated to BAR, and thus gets stuck with a value of FOO.
        /// </remarks>
        public bool BroadcastPropsChangeToAll { get; set; } = true;
    ...
    {
        /// <summary>
        ///     Default options: CachingOption: DoNotCache, InterestGroup: 0, targetActors: null, receivers: Others,
        ///     sequenceChannel: 0.
        /// </summary>
        public static readonly RaiseEventOptions Default = new();

        /// <summary>
        ///     Defines if the server should simply send the event, put it in the cache or remove events that are like this
        ///     one.
        /// </summary>
        /// <remarks>
        ///     When using option: SliceSetIndex, SlicePurgeIndex or SlicePurgeUpToIndex, set a CacheSliceIndex. All other options
        ...
        /// <summary>
        ///     The number of the Interest Group to send this to. 0 goes to all users but to get 1 and up, clients must
        ///     subscribe to the group first.
        /// </summary>
        public byte InterestGroup;

        /// <summary>
        ///     Sends the event to All, MasterClient or Others (default). Be careful with MasterClient, as the client might
        ///     disconnect before it got the event and it gets lost.
        /// </summary>
        public ReceiverGroup Receivers;

        /// <summary>
        ///     Events are ordered per "channel". If you have events that are independent of others, they can go into another
        ///     sequence or channel.
        /// </summary>
        ...
        /// <summary>
        ///     A list of Player.ActorNumbers to send this event to. You can implement events that just go to specific users
        ///     this way.
        /// </summary>
    ...
    {
        /// <summary>
        ///     Standard type and behaviour: While joined to this lobby clients get room-lists and JoinRandomRoom can use a
        ///     simple filter to match properties (perfectly).
        /// </summary>
        Default = 0,

        /// <summary>
        ///     This lobby type lists rooms like Default but JoinRandom has a parameter for SQL-like "where" clauses for
        ///     filtering. This allows bigger, less, or and and combinations.
        /// </summary>
        SqlLobby = 2,

        /// <summary>
        ///     This lobby does not send lists of games. It is only used for OpJoinRandomRoom. It keeps rooms available for a
        ///     while when there are only inactive users left.
        /// </summary>
        AsyncRandomLobby = 3
    }
    ...
    public class TypedLobby
    {
        ...
        /// </summary>
        /// <remarks>
        ///     If Name is null or empty, a TypedLobby will point to the "default lobby". This ignores the Type value and always
        ///     acts as  <see cref="LobbyType.Default" />.
        /// </remarks>
        public string Name;
        ...
        /// <summary>
        ///     Sets Name and Type of the new instance. Make sure name is not empty or null, as that always points to the "default
        ///     lobby" (<see cref="TypedLobby.Default" />).
        /// </summary>
    ...
    public class TypedLobbyInfo : TypedLobby
    {
        ...
        public override string ToString()
        {
            return string.Format("TypedLobbyInfo '{0}'[{1}] rooms: {2} players: {3}", Name, Type, RoomCount,
                PlayerCount);
        }
    }
    ...
    public enum CustomAuthenticationType : byte
    {
        ...
        /// <summary>
        ///     Authenticates users by their Oculus Account and token. Set Oculus' userid as "userid" and nonce as "nonce" via
        ///     AddAuthParameter().
        /// </summary>
        Oculus = 3,

        /// <summary>
        ///     Authenticates users by their PSN Account and token on PS4. Set token as "token", env as "env" and userName as
        ///     "userName" via AddAuthParameter().
        /// </summary>
        PlayStation4 = 4,
        ...
        /// <summary>
        ///     Authenticates users by their NSA ID. Set token  as "token" and appversion as "appversion" via
        ///     AddAuthParameter(). The appversion is optional.
        /// </summary>
        NintendoSwitch = 11,

        /// <summary>
        ///     Authenticates users by their PSN Account and token on PS5. Set token as "token", env as "env" and userName as
        ///     "userName" via AddAuthParameter().
        /// </summary>
        PlayStation5 = 12,
        ...
        /// <summary>
        ///     Authenticates users with Epic Online Services (EOS). Set token as "token" and ownershipToken as
        ///     "ownershipToken" via AddAuthParameter(). The ownershipToken is optional.
        /// </summary>
        Epic = 13,
        ...
        /// <summary>
        ///     Disables custom authentication. Same as not providing any AuthenticationValues for connect (more precisely
        ///     for: OpAuthenticate).
        /// </summary>
        None = byte.MaxValue
    }
    ...
    public class AuthenticationValues
    {
        ...
        /// <remarks>
        ///     Several auth providers are available and CustomAuthenticationType.Custom can be used if you build your own
        ///     service.
        /// </remarks>
        ...
        /// <summary>
        ///     Internal <b>Photon token</b>. After initial authentication, Photon provides a token for this client,
        ///     subsequently used as (cached) validation.
        /// </summary>
        ...
        /// <remarks>
        ///     AuthPostData is just one value. Each SetAuthPostData replaces any previous value. It can be either a string, a
        ///     byte[] or a dictionary.
        /// </remarks>
        /// <param name="stringData">
        ///     String data to be used in the body of the POST request. Null or empty string will set
        ///     AuthPostData to null.
        /// </param>
        ...
        /// </remarks>
        /// <param name="dictData">
        ///     A authentication-data dictionary will be converted to Json and passed to the Auth webservice via
        ///     HTTP Post.
        /// </param>
        ...
        {
            var ampersand = string.IsNullOrEmpty(AuthGetParameters) ? "" : "&";
            AuthGetParameters = string.Format("{0}{1}{2}={3}", AuthGetParameters, ampersand, Uri.EscapeDataString(key),
        ...
        public override string ToString()
        {
            return string.Format("AuthenticationValues = AuthType: {0} UserId: {1}{2}{3}{4}",
                AuthType,
                UserId,
                string.IsNullOrEmpty(AuthGetParameters) ? " GetParameters: yes" : "",
                AuthPostData == null ? "" : " PostData: yes",
