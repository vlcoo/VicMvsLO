...
namespace Photon.Realtime
{
    ...
    public class Player
    {
        ...
        ///     ID or ActorNumber of this player in the current room (a shortcut to identify each player in
        ///     room)
        /// </param>
        ...
        ///     A Hashtable of custom properties to be synced. Must use String-typed keys and
        ///     serializable datatypes as values.
        /// </param>
        ...
        /// <summary>
        ///     Identifier of this player in current room. Also known as: actorNumber or actorNumber. It's -1 outside of
        ///     rooms.
        /// </summary>
        /// <remarks>
        ///     The ID is assigned per room and only valid in that context. It will change even on leave and re-join. IDs are
        ///     never re-used per room.
        /// </remarks>
        ...
        public string NickName
        {
            ...
            set
            {
                if (!string.IsNullOrEmpty(nickName) && nickName.Equals(value)) return;

                nickName = value;

                // update a room, if we changed our nickName locally
                if (IsLocal) SetPlayerNameProperty();
            }
        }

        /// <summary>UserId of the player, available when the room got created with RoomOptions.PublishUserId = true.</summary>
        /// <remarks>
        ///     Useful for <see cref="LoadBalancingClient.OpFindFriends" /> and blocking slots in a room for expected players
        ///     (e.g. in <see cref="LoadBalancingClient.OpCreateRoom" />).
        /// </remarks>
        ...
        public bool IsMasterClient
        {
            get
            {
                if (RoomReference == null) return false;
        ...
        /// <remarks>
        ///     Inactive players keep their spot in a room but otherwise behave as if offline (no matter what their actual
        ///     connection status is).
        ///     The room needs a PlayerTTL != 0. If a player is inactive for longer than PlayerTTL, the server will remove this
        ///     player from the room.
        ///     For a client "rejoining" a room, is the same as joining it: It gets properties, cached events and then the live
        ///     events.
        /// </remarks>
        ...
        {
            if (RoomReference == null || RoomReference.Players == null || RoomReference.Players.Count < 2) return null;
        ...
        /// <summary>
        ///     Caches properties for new Players or when updates of remote players are received. Use SetCustomProperties()
        ///     for a synced update.
        /// </summary>
        /// <remarks>
        ...
        {
            if (properties == null || properties.Count == 0 || CustomProperties.Equals(properties)) return;
            ...
            if (properties.ContainsKey(ActorProperties.PlayerName))
            {
                ...
                if (nameInServersProperties != null)
                {
                    if (IsLocal)
                    {
                        ...
                        if (!nameInServersProperties.Equals(nickName)) SetPlayerNameProperty();
                    }
                    else
                    {
            ...
            if (properties.ContainsKey(ActorProperties.UserId)) UserId = (string)properties[ActorProperties.UserId];
        ...
        {
            return string.Format("#{0:00} '{1}'", ActorNumber, NickName);
        }
        ...
        {
            return string.Format("#{0:00} '{1}'{2} {3}", ActorNumber, NickName, IsInactive ? " (inactive)" : "",
        ...
        /// <summary>
        ///     Updates and synchronizes this Player's Custom Properties. Optionally, expectedProperties can be provided as
        ///     condition.
        /// </summary>
        /// <remarks>
        ...
        ///     If non-null, these are the property-values the server will check as condition for this
        ///     update.
        /// </param>
        /// <param name="webFlags">
            ...
            WebFlags webFlags = null)
        {
            ...
            if (RoomReference != null)
            {
                if (RoomReference.IsOffline)
                {
                    ...
                    CustomProperties.StripKeysWithNullValues();
                    // invoking callbacks
                    RoomReference.LoadBalancingClient.InRoomCallbackTargets.OnPlayerPropertiesUpdate(this, customProps);
                    return true;
                }

                var customPropsToCheck = expectedValues.StripToStringKeys();

                // send (sync) these new values if in online room
                return RoomReference.LoadBalancingClient.OpSetPropertiesOfActor(actorNumber, customProps,
        ...
        private bool SetPlayerNameProperty()
        {
            if (RoomReference != null && !RoomReference.IsOffline)
            {
                ...
                properties[ActorProperties.PlayerName] = nickName;
                return RoomReference.LoadBalancingClient.OpSetPropertiesOfActor(ActorNumber, properties);
            }
