...
namespace Photon.Pun
{
    ...
    public static partial class PhotonNetwork
    {
        ...
        private static readonly HashSet<PhotonView> reusablePVHashset = new();
        ...
        /// <summary>
        ///     While enabled, the MonoBehaviours on which we call RPCs are cached, avoiding costly GetComponents&lt;MonoBehaviour
        ///     &gt;() calls.
        /// </summary>
        /// <remarks>
        ...
        private static readonly Hashtable rpcEvent = new();

        private static readonly RaiseEventOptions RpcOptionsToAll = new();
        ...
        private static readonly RaiseEventOptions serializeRaiseEvOptions = new();
        ...
        /// <summary>
        ///     An Object Pool can be used to keep and reuse instantiated object instances. Replaces Unity's default Instantiate
        ///     and Destroy methods.
        /// </summary>
        /// <remarks>
        ...
        /// </summary>
        /// <remarks>
        ///     if (Server == ServerConnection.GameServer && (state == ClientState.Disconnecting || state ==
        ...
        /// <summary>
        ///     Resets the PhotonView "lastOnSerializeDataSent" so that "OnReliable" synched PhotonViews send a complete state to
        ///     new clients (if the state doesnt change, no messages would be send otherwise!).
            ...
            for (var componentsIndex = 0; componentsIndex < photonNetview.RpcMonoBehaviours.Length; componentsIndex++)
            {
                ...
                for (var index = 0; index < cachedRPCMethods.Count; index++)
                {
                    ...
                    if (parameters.Length == arguments.Length)
                    {
                        ...
                        {
                            receivers++;
                            var o = mInfo.Invoke(monob, arguments);
                            if (RunRpcCoroutines)
                            {
                                IEnumerator ie = null; //o as IEnumerator;
                                if ((ie = o as IEnumerator) != null) PhotonHandler.Instance.StartCoroutine(ie);
                    ...
                    if (parameters.Length == arguments.Length + 1)
                    {
                            ...
                            CheckTypeMatch(parameters, argumentsTypes))
                        {
                            ...
                            receivers++;
                            var o = mInfo.Invoke(monob, argumentsWithInfo);
                            if (RunRpcCoroutines)
                            {
                                IEnumerator ie = null; //o as IEnumerator;
                                if ((ie = o as IEnumerator) != null) PhotonHandler.Instance.StartCoroutine(ie);
                    ...
                    {
                        receivers++;
                        var o = mInfo.Invoke(monob, new object[] { arguments });
                        if (RunRpcCoroutines)
                        {
                            IEnumerator ie = null; //o as IEnumerator;
                            if ((ie = o as IEnumerator) != null) PhotonHandler.Instance.StartCoroutine(ie);
        ...
        /// <summary>
        ///     Check if all types match with parameters. We can have more paramters then types (allow last RPC type to be
        ///     different).
        /// </summary>
        ...
        /// <summary>
        ///     Destroys all Instantiates and RPCs locally and (if not localOnly) sends EvDestroy(player) and clears related events
        ...
        /// <summary>
        ///     Removes GameObject and the PhotonViews on it from local lists and optionally updates remotes. GameObject gets
        ///     destroyed at end.
        /// </summary>
        /// <remarks>
        ...
        private static void OpRemoveFromServerInstantiationsOfPlayer(int actorNr)
        {
            ...
            var options = new RaiseEventOptions
                { CachingOption = EventCaching.RemoveFromRoomCache, TargetActors = new[] { actorNr } };
        ...
        internal static void RequestOwnership(int viewID, int fromOwner)
        {
            ...
            RaiseEventInternal(PunEvent.OwnershipRequest, new[] { viewID, fromOwner }, SendToAllOptions,
        ...
        internal static void TransferOwnership(int viewID, int playerID)
        {
            ...
            RaiseEventInternal(PunEvent.OwnershipTransfer, new[] { viewID, playerID }, SendToAllOptions,
                SendOptions.SendReliable);
        }

        /// <summary>
        ///     Call this on the Master to reassert ownership on clients. viewOwnerPairs are [viewId][viewOwnerActorNr] pairs.
        ///     targetActor of -1 indicates send to all others.
        /// </summary>
        ...
        {
            var options = new RaiseEventOptions
                { CachingOption = EventCaching.RemoveFromRoomCache, TargetActors = new[] { actorNumber } };
        ...
        private static void RemoveCacheOfLeftPlayers()
        {
            ...
            NetworkingClient.LoadBalancingPeer.SendOperation(OperationCode.RaiseEvent, opParameters,
        ...
        /// <summary>
        ///     Remove all buffered RPCs from server that were sent in the targetGroup, if this is the Master Client or if this
        ///     controls the individual PhotonView.
        /// </summary>
        /// <remarks>
        ...
        /// </summary>
        /// <param name="viewId">
        ///     The viewID of the PhotonView where the RPC has been called on. We actually need its ViewID. If 0
        ///     (default) is provided, all PhotonViews/ViewIDs are considered.
        /// </param>
        /// <param name="methodName">
        ///     The RPC method name, if possible we will use its hash shortcut for efficiency. If none (null
        ///     or empty string) is provided all RPC method names are considered.
        /// </param>
        /// <param name="callersActorNumbers">
        ///     The actor numbers of the players who called/buffered the RPC. For example if two
        ///     players buffered the same RPC you can clear the buffered RPC of one and keep the other. If none (null or empty
        ///     array) is provided all senders are considered.
        /// </param>
            ...
            params object[] parameters)
        {
            ...
            rpcEvent[keyByteZero] = view.ViewID; // LIMITS NETWORKVIEWS&PLAYERS
            ...
            if (parameters != null && parameters.Length > 0) rpcEvent[keyByteFour] = parameters;
            ...
            if (player != null)
            {
                ...
                {
                    var options = new RaiseEventOptions { TargetActors = new[] { player.ActorNumber } };
            ...
            switch (target)
            {
                ...
                case RpcTarget.All:
                    RpcOptionsToAll.InterestGroup =
                        view.Group; // NOTE: Test-wise, this is static and re-used to avoid memory garbage
                ...
                {
                    var options = new RaiseEventOptions { InterestGroup = view.Group };
                    RaiseEventInternal(PunEvent.RPC, rpcEvent, options, sendOptions);
                    break;
                }
                ...
                {
                    var options = new RaiseEventOptions { InterestGroup = view.Group, Receivers = ReceiverGroup.All };
                    RaiseEventInternal(PunEvent.RPC, rpcEvent, options, sendOptions);
                ...
                case RpcTarget.AllBufferedViaServer:
                {
                    var options = new RaiseEventOptions
                    {
                        InterestGroup = view.Group, Receivers = ReceiverGroup.All,
        ...
        /// <summary>
        ///     Calls all locally controlled PhotonViews to write their updates in OnPhotonSerializeView. Called by a
        ///     PhotonHandler.
        /// </summary>
        ...
        private static void SendSerializeViewBatch(SerializeViewBatch batch)
        {
            ...
            batch.ObjectUpdates[1] = currentLevelPrefix != 0 ? currentLevelPrefix : null;
        ...
        private class SerializeViewBatch : IEquatable<SerializeViewBatch>, IEquatable<RaiseEventBatch>
        {
            ...
            private readonly int defaultSize = ObjectsInOneUpdate;
