...
namespace Photon.Realtime
{
    ...
    public class AccountService
    {
        ...
        /// </summary>
        public bool RequestPendingResult;
        ...
        {
            var emailEscaped = UnityWebRequest.EscapeURL(email);
            var st = UnityWebRequest.EscapeURL(serviceTypes);
            var uv = UnityWebRequest.EscapeURL(Application.unityVersion);
            var serviceUrl = string.Format(ServiceUrl,
            ...
            try
            {
                ...
                if (res.ReturnCode == AccountServiceReturnCodes.Success)
                {
                    ...
                    if (!string.IsNullOrEmpty(applicationIds))
                    {
                        ...
                        for (var i = 0; i < parts.Length; i = i + 2) res.ApplicationIds.Add(parts[i], parts[i + 1]);
                    }
                    else
                    {
                        Debug.LogError(
        ...
        {
            if (appTypes == null || appTypes.Count <= 0) return null;
