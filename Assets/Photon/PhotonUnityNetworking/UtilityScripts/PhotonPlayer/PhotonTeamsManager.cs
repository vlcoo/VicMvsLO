...
#pragma warning restore 0414
        ...
        [SerializeField] private List<PhotonTeam> teamsList = new()
        {
            ...
            if (changedProps.TryGetValue(TeamPlayerProp, out temp))
            {
                if (temp == null)
                {
                    foreach (var code in playersPerTeam.Keys)
                        ...
                        {
                            if (PlayerLeftTeam != null) PlayerLeftTeam(targetPlayer, teamsByCode[code]);
                            break;
                        }
                }
                else if (temp is byte)
                {
                    ...
                    if (PlayerJoinedTeam != null) PlayerJoinedTeam(targetPlayer, team);
                }
                else
                {
        ...
        private void UpdateTeams()
        {
            ...
            for (var i = 0; i < PhotonNetwork.PlayerList.Length; i++)
            {
                ...
                if (playerTeam != null) playersPerTeam[playerTeam.Code].Add(player);
            }
        }

        private void ClearTeams()
        {
            foreach (var key in playersPerTeam.Keys) playersPerTeam[key].Clear();
        }
        ...
        {
            if (teamsList != null) return teamsList.ToArray();
            return null;
        }
        ...
        public bool TryGetTeamMembers(byte code, out Player[] members)
        {
            ...
            if (playersPerTeam.TryGetValue(code, out players))
            {
        ...
        public bool TryGetTeamMembers(string teamName, out Player[] members)
        {
            members = null;
            PhotonTeam team;
            if (TryGetTeamByName(teamName, out team)) return TryGetTeamMembers(team.Code, out members);
            return false;
        }
        ...
        {
            members = null;
            if (team != null) return TryGetTeamMembers(team.Code, out members);
            return false;
        }
        ...
        public bool TryGetTeamMatesOfPlayer(Player player, out Player[] teamMates)
        {
            ...
            HashSet<Player> players;
            if (playersPerTeam.TryGetValue(team.Code, out players))
            {
        ...
        {
            PhotonTeam team;
            if (TryGetTeamByCode(code, out team)) return GetTeamMembersCount(team);
            return 0;
        }
        ...
        {
            PhotonTeam team;
            if (TryGetTeamByName(name, out team)) return GetTeamMembersCount(team);
            return 0;
        }
        ...
        {
            HashSet<Player> players;
            if (team != null && playersPerTeam.TryGetValue(team.Code, out players) && players != null)
    ...
    public static class PhotonTeamExtensions
    {
        ...
        public static PhotonTeam GetPhotonTeam(this Player player)
        {
            ...
            if (player.CustomProperties.TryGetValue(PhotonTeamsManager.TeamPlayerProp, out teamId) &&
                PhotonTeamsManager.Instance.TryGetTeamByCode((byte)teamId, out team)) return team;
            return null;
        }
        ...
        /// <returns>
        ///     If the team switch request is queued to be sent to the server or done in case offline or not joined to a room
        ///     yet.
        /// </returns>
        ...
        /// <param name="player"></param>
        /// <returns>
        ///     If the leaving team request is queued to be sent to the server or done in case offline or not joined to a room
        ///     yet.
        /// </returns>
