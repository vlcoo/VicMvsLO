...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class CullAreaEditor : Editor
    {
        ...
        public void OnSceneGUI()
        {
            ...
            if (GUILayout.Button("Reset position")) cullArea.transform.position = Vector3.zero;

            if (GUILayout.Button("Reset scaling")) cullArea.transform.localScale = new Vector3(25.0f, 25.0f, 25.0f);
...

            {
                ...
                if (cullArea.NumberOfSubdivisions != 0)
                    for (var index = 0; index < cullArea.Subdivisions.Length; ++index)
                        ...
                        {
                            cullArea.Subdivisions[index] = new Vector2(1, 1);
                        }
            }
...

            {
                ...
                if (Camera.main != null)
                {
                    if (GUILayout.Button("Align main camera with grid"))
                    {
                        ...
                        var yCoord = cullArea.YIsUpAxis
                            ? cullArea.Center.y
                            : Mathf.Max(cullArea.Size.x, cullArea.Size.y);
                        var zCoord = cullArea.YIsUpAxis
                            ? -Mathf.Max(cullArea.Size.x, cullArea.Size.y)
                            : cullArea.Center.y;
                    ...
                    EditorGUILayout.LabelField("Current main camera position is " + Camera.main.transform.position);
                }
        ...
        private void AlignEditorView()
        {
            ...
            if (SceneView.lastActiveSceneView != null) SceneView.lastActiveSceneView.AlignViewToObject(tmpGo.transform);
