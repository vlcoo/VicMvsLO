...
public class CustomRigidbodySerializer : MonoBehaviourPun, ICustomSerializeView
{
    ...
    public void Serialize(List<byte> buffer)
    {
        ...
        var sendVelocity = body.bodyType != RigidbodyType2D.Static &&
                           Vector2.Distance(previousVelocity, body.velocity) > EPSILON;
        var sendPosition = Vector2.Distance(previousPosition, body.position) > EPSILON;
        ...
        {
            SerializationUtils.PackToInt(buffer, body.position, GameManager.Instance.GetLevelMinX() - 0.5f,
                GameManager.Instance.GetLevelMaxX() + 0.5f, -20, 20);
    ...
    {
        //position
        SerializationUtils.UnpackFromInt(buffer, ref index, GameManager.Instance.GetLevelMinX() - 0.5f,
            GameManager.Instance.GetLevelMaxX() + 0.5f, out var newPosition, -20, 20);

        //velocity
        var syncVelocity = body.bodyType != RigidbodyType2D.Static;
        if (syncVelocity)
        {
            SerializationUtils.UnpackFromInt(buffer, ref index, -20, 20, out var newVelocity);
        ...
        if (syncVelocity && interpolate)
        {
            ...
            if ((interpDistance = Vector2.Distance(interpPosition, body.position)) > TELEPORT_DISTANCE ||
                interpDistance < EPSILON)
