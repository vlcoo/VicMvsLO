...
public abstract class InteractableTile : AnimatedTile
{
    ...
    private static readonly Vector3 bumpOffset = new(0.25f, 0.5f, 0);
    private static readonly Vector3 bumpSize = new(0.45f, 0.1f, 0);
    ...
    public static void Bump(MonoBehaviour interacter, InteractionDirection direction, Vector3 worldLocation)
    {
        ...
        foreach (var collider in Physics2D.OverlapBoxAll(worldLocation + bumpOffset, bumpSize, 0f))
        {
            ...
            switch (obj.tag)
            {
                case "Player":
                {
                    ...
                    player.photonView.RPC("Knockback", RpcTarget.All,
                        obj.transform.position.x < interacter.transform.position.x, 1, false,
                        (interacter as MonoBehaviourPun)?.photonView.ViewID ?? -1);
                    continue;
                }
                case "koopa":
                {
                ...
                case "goomba":
                {
                    if (!obj.GetPhotonView())
                        continue;
                    obj.GetPhotonView().RPC(nameof(KillableEntity.SpecialKill), RpcTarget.All,
                        obj.transform.position.x < worldLocation.x, false, 0);
                    continue;
                }
                case "loosecoin":
                case "coin":
                {
                    ...
                    if (interacter is PlayerController pl)
                        pl.photonView.RPC(nameof(PlayerController.AttemptCollectCoin), RpcTarget.All,
                            view.GetComponentInParent<PhotonView>().ViewID, (Vector2)obj.transform.position);
                    continue;
                }
                case "MainStar":
                case "bigstar":
