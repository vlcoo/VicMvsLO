...
public class SpinyWalk : KoopaWalk
{
    public override void InteractWithPlayer(PlayerController player)
    {
        ...
        var attackedFromAbove = Vector2.Dot(damageDirection, Vector2.up) > 0f;
        if (holder)
            return;
                 ...
                 player.state == Enums.PowerupState.MegaMushroom)
        {
            ...
            if (player.inShell && shell && !stationary &&
                Mathf.Sign(body.velocity.x) != Mathf.Sign(player.body.velocity.x))
                //Do knockback to player, colliding with us in shell going opposite ways
                player.photonView.RPC("Knockback", RpcTarget.All, player.body.position.x < body.position.x, 0,
                    photonView.ViewID);
                    ...
                    if (attackedFromAbove)
                    {
                        ...
                        else
                        {
                            ...
                            if (player.groundpound)
                            {
                                ...
                                photonView.RPC("Kick", RpcTarget.All, player.body.position.x < body.position.x,
                                    Mathf.Abs(player.body.velocity.x) / player.RunningMaxSpeed, true);
                                player.photonView.RPC("SetHoldingOld", RpcTarget.All, photonView.ViewID);
                            }
                            else
                            {
