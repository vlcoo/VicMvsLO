...
public class FrozenCube : HoldableEntity
{
    ...
    public new void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        if (killa)
        {
            ...
            killa.photonView.RPC(nameof(KillableEntity.SpecialKill), RpcTarget.All,
                killa.transform.position.x > transform.position.x, false, combo++);
        }
        ...
        switch (obj.tag)
        {
            case "coin":
            {
                (holder != null ? holder : previousHolder).photonView.RPC(nameof(PlayerController.AttemptCollectCoin),
                    RpcTarget.All, obj.GetPhotonView().ViewID, (Vector2)obj.transform.position);
                break;
            }
            case "loosecoin":
            {
                ...
                (holder != null ? holder : previousHolder).photonView.RPC(nameof(PlayerController.AttemptCollectCoin),
                    RpcTarget.All, parent.gameObject.GetPhotonView().ViewID, (Vector2)parent.position);
                break;
            }
        }
    }
    ...
    public new void Start()
    {
        ...
        {
            var id = (int)photonView.InstantiationData[0];
            ...
            var rendererObject = entityView.gameObject;
            ...
            entityPositionOffset =
                -(bounds.center - Vector3.up.Multiply(bounds.size / 2) - rendererObject.transform.position);
    ...
    public override void FixedUpdate()
    {
        ...
        if (photonView.IsMine && (body.position.y + hitbox.size.y < GameManager.Instance.GetLevelMinY() ||
                                  Utils.IsTileSolidAtWorldLocation(body.position + hitbox.size.y / 2f * Vector2.up)))
        {
            entityView.RPC(nameof(IFreezableEntity.Unfreeze), RpcTarget.All,
                (byte)IFreezableEntity.UnfreezeReason.Other);
            PhotonNetwork.Destroy(photonView);
            return;
        }

        if (photonView.IsMine && holder && Utils.IsAnyTileSolidBetweenWorldBox(body.position + hitbox.offset,
                hitbox.size * transform.lossyScale * 0.75f))
        {
        ...
        if (!fastSlide && autoBreakTimer < 1f)
            body.position =
                new Vector2(
                    body.position.x + Mathf.Sin(autoBreakTimer * shakeSpeed) * shakeAmount * Time.fixedDeltaTime,
        ...
        {
            var ray = Physics2D.BoxCast(body.position + Vector2.up * hitbox.size / 2f, hitbox.size, 0, Vector2.down,
                0.2f, Layers.MaskOnlyGround);
            if (ray)
            {
    ...
    public override void InteractWithPlayer(PlayerController player)
    {
        ...
        if ((player.groundpound || player.groundpoundLastFrame) && attackedFromAbove &&
            player.state != Enums.PowerupState.MiniMushroom)
        {
        ...
        {
            player.photonView.RPC(nameof(PlayerController.Knockback), RpcTarget.All,
                body.position.x > player.body.position.x, 1, false, photonView.ViewID);
            photonView.RPC(nameof(Kill), RpcTarget.All);
        }
